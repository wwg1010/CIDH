...
def triplet_1(y,u):         # https://arxiv.org/pdf/1612.03900.pdf
    inner_product = u @ u.t()
    s = y @ y.t() > 0
    count = 0
    ...
    for row in range(s.shape[0]):
        ...
        if s[row].sum() != 0 and (~s[row]).sum() != 0:
            ...
            triple = (theta_positive.unsqueeze(1) - theta_negative.unsqueeze(0) - 20).clamp(min=-100, max=50)
